...
impl Store {
    ...
    /// Delegates to [`Store::record_tool_usage_with_basis`] with an empty `served_basis`
    /// (kept for callers that don't distinguish a serving basis); the row reads
    /// back as "unspecified" in [`Store::usage_by_basis`].
    ...
    /// [`Store::usage_by_basis`] split the aggregate. `surface` is 'mcp' | 'web' | 'cli' (no
    /// DB CHECK — see the schema comment). Every ~1000th insert opportunistically GCs rows
    /// past the retention window so the table can't grow unbounded.
    ...
    /// record answer+citations), so the single [`Store::usage_summary`] figure mixes them;
    /// this splits it so `status` / `/api/impact` can reconcile per-surface. `NULL`/empty tags
    /// (legacy rows, untagged callers — including every MCP row until that half is tagged too)
    /// group under `"unspecified"`.
