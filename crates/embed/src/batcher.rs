...
//! store. The caller drains it (`batch_refs`), runs `embed_all` itself — so a future
//! integration can fire its own memory watchdog immediately before the big embed — and
//! hands the results back (`scatter`) to be routed to the owning chunks. A file is returned
...
//! embed-text rides in. Every retrieval consumer is cosine and scale-invariant (brute-force
//! cosine + the HNSW `AnnIndex`'s `DistCosine`), and the Ollama client already stores a mix
//! of L2-normalized and raw vectors by design that "rank identically" — so reordering which
//! files' misses share a batch is rank-neutral. The bar is retrieval-equivalence, not
//! byte-identity.
...
mod tests {
    ...
    /// meta/tag). Mirrors what a deep loop would hand `add_file`.
    type TestFile = (Vec<Option<Vec<f32>>>, Vec<(usize, String)>, String);

    /// Drive files through the batcher the way a caller is expected to: add each, flush on
    ...
    //     i.e. accumulating below the threshold does NOT trigger a premature flush, and
    //     hitting the threshold does.
    #[tokio::test]
    ...
    //     results are not scrambled across batch boundaries.
    #[tokio::test]
    ...
    // (g) Below-threshold files finalize only at the tail (explicit finalize) flush — proves
    //     a final partial batch does NOT flush on its own until the caller asks for it.
    #[tokio::test]
    ...
    // (h) `buffered()` reports the live count so a caller can decide whether a tail flush is
    //     worth doing without depending on internal state.
    #[test]
    fn buffered_reports_live_count() {
        let mut b = MissBatcher::<()>::new(4, 64);
        assert_eq!(b.buffered(), 0);
        assert!(b.is_empty());
        assert!(matches!(
            b.add_file(vec![None, None], vec![(0, "a".into()), (1, "b".into())], ()),
            AddOutcome::Buffered
        ));
