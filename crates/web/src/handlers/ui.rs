...
/// A strong `ETag` = a hex FNV-1a hash of the embedded asset bytes, quoted. The bundle is
/// embedded at compile time, so each hash is constant per build — a repeat load whose
/// `If-None-Match` matches gets a `304 Not Modified` (no re-download of the bundle) while
/// `Cache-Control: no-cache` still forces a revalidation on every navigation, so a new build is
/// always picked up.
...
/// Serve an embedded text asset with a content-hash `ETag` + `Cache-Control: no-cache`,
/// returning `304 Not Modified` (no body) when the client's `If-None-Match` already matches.
fn cached_asset(
    headers: &HeaderMap,
    etag: &str,
