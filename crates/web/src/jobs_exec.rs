...
use indexa_query::{
    contextual::{build_blurb_prompt, build_doc_context},
    process_queue_item_with_passes, requeue_subtree, QueueOutcome, MAX_DIR_DEFERS,
};
                ...
                for chunk in &extracted.chunks {
                    ...
                    let embed_text =
                        if let (Some(ref llm), Some(ref doc)) = (&ctx_llm, &doc_context) {
                            // Use the shared prompt builder — single source of truth.
                            let prompt = build_blurb_prompt(doc, &chunk.text);
