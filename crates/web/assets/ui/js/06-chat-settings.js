...
/* Render the Sources block appended below an answer. Each item carries data-path so the
   delegated handler below can open its summary — clicking (or Enter/Space on) a cited
   source jumps to it, the same way a tree/search result does. */
...
// Delegated: a cited source is clickable (and keyboard-activatable) → open its summary via the
// same showSummary() the tree/search results use. Bound once on the document because source
// items are re-painted during streaming, matching the .explain-load-btn handler below.
...
async function doAsk() {
  if (askInFlight) return; // a stream is already open — Stop must abort it, not stack a second ask
  ...
  // stream — see the handler at the bottom of this file). The AbortController cancels both the
  // fetch and the in-progress reader loop below.
  askInFlight = true;
  ...
  // Skipped when the user stopped the stream early — a thin result at that point is their
  // choice (they cut it off), not evidence the scope was a dead-end.
...
// Enter starts an ask, but never a second one while one is already streaming (doAsk() also
// no-ops in that case — this just avoids clearing qInput's Enter-repeat feel for nothing).
