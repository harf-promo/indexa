...
impl IndexaMcp {
    ...
    async fn ensure_ann(&self) -> Option<Arc<AnnIndex>> {
        ...
        // (halves transient build memory). Dim comes from the first stored vector (all stored
        // vectors share it), so the built index matches the old collect-then-build set.
