...
- **Four ways to reach the index** — a CLI, a live web workspace with an Engine status bar, a native macOS desktop app (menu-bar, auto-updating), and an MCP server (53 tools) that AI agents call directly.
...
- **MCP server** — `indexa mcp` exposes the live index to any [MCP](https://modelcontextprotocol.io) client (Claude Desktop, Cursor, Claude Code) over **53 tools** — retrieval (`search · browse_tree · get_summary · read_file · get_chunk_context · ask`, with multi-turn `session_id`), project overview + retrieval trace + path inspection, code graph (`dependencies · who_imports · who_calls · blast_radius · code_graph · changed_impact · trace_path · symbol_context · dependency_closure`), Context Packs, importance weights, smart classification, insights (including summarize-pass drift), decision review (with durable, patch-id-anchored ledger notes), config introspection, and indexing triggers — plus **4 read-only resources** (`indexa://overview · packs · pack/{name} · summary/{path}`) and **3 prompt templates** (`onboarding-overview · explain-file · pack-context`).
