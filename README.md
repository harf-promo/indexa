...
- **Four ways to reach the index** — a CLI, a live web workspace with an Engine status bar, a native macOS desktop app (menu-bar, auto-updating), and an MCP server (56 tools) that AI agents call directly.
...
- **MCP server** — `indexa mcp` exposes the live index to any [MCP](https://modelcontextprotocol.io) client (Claude Desktop, Cursor, Claude Code) over **56 tools** — retrieval (`search · browse_tree · get_summary · read_file · get_chunk_context · ask`, with multi-turn `session_id`), project overview + retrieval trace + path inspection, code graph (`dependencies · who_imports · who_calls · blast_radius · code_graph · changed_impact · trace_path · symbol_context · dependency_closure`), Context Packs, importance weights, smart classification, insights (including summarize-pass drift), decision review (with durable, patch-id-anchored ledger notes), durable memory (`memory_record · memory_search · memory_update` — typed claims with a confidence ceiling on agent-authored ones), config introspection, and indexing triggers — plus **4 read-only resources** (`indexa://overview · packs · pack/{name} · summary/{path}`) and **3 prompt templates** (`onboarding-overview · explain-file · pack-context`).
...
Recently shipped: **remote plugin directory** (`indexa plugin list --refresh` fetches the curated list off `main` instead of only the copy embedded in the binary; distribution stays compile-time either way — see [CONTRIBUTING.md](CONTRIBUTING.md#authoring-a-third-party-parser-plugin)), **agent-session search** (index Claude Code's own JSONL transcripts, opt-in), **`indexa eval --judge`** (LLM-graded answer quality alongside retrieval-ranking metrics), **Cerebras** as an opt-in high-throughput provider, **Plugin directory** (`indexa plugin list`/`info` over `crates/parsers/plugins.toml`), **Context Pack inclusion modes** (`reference`/`pinned`) with export `--dry-run` and typed redaction labels, **Map Architecture-layers overlay**, **durable decision-ledger notes** (`record_decision`) and **summarize-pass drift** reporting, cross-file embed batching, **agentic `ask`** (multi-hop plan→search→refine), **PageRank centrality** (hub files in the code graph), **signature graph visualization**, **universal macOS desktop build**, **Importance weighting**, **Insights** (duplicates / stale / weekly diff).
