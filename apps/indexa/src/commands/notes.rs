...
pub(crate) async fn cmd_note_add_at(
    ...
    // mirroring `cmd_pack_refresh_at`'s reindex call. `mode: None` defers to the configured
    // `[describer] mode` (same as `cmd_pack_refresh_at` — forcing a mode here would silently
    // override the user's config).
    match cmd_deep(vec![note_path], None, false, None, false, false, false, cfg).await {
        Ok(()) => println!("  Indexed."),
        Err(e) => println!(
            "  \u{26a0} Indexing the note failed ({e:#}) — run `indexa pack refresh \"{pack}\"` \
             once the embedder is reachable."
        ),
    }
    Ok(())
}
